...
public class Main {
    public static void main(String[] args) throws IOException{
        ...
        // int[] arr = {3, 4, 5};
        // System.out.println(assemblyNumber(arr, 521, 3)); // => 543
        // System.out.println(Arrays.toString(constructDigits(4, 3, 2))); // => [6, 5, 4]
        // System.out.println(Arrays.toString(constructDigits(4, 3, 1))); // => [6, 5, 4]
    ...
    public static int[] constructDigits(int biggerDigit, int length, int shifft) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            // if(result[i] == -1)
            // {
            //     System.err.println("COMO ASSIM? NEGATIVO????");
            // }
        }
    ...
    public static int assemblyNumber(int[] digits, int year, int length){
        ...
        for (int i = 0; i < length && !greaterFound; i++) {
            ...
            for(int j = i; j < length; j++){
                if(!digitUsed[j] && digits[i] > currentYearDigit){
                    select = digits[i];
                    digitFound = true;
                    digitUsed[j] = true;
                    greaterFound = true;
