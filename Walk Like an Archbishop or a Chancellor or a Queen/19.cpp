...
// Informacao:
// VERSAO2: Tentei colocar com numeros de erros, uma vez que a peça C que tenta fazer um melhor resultado que a peça A e a peça C tem mais erros que a peça A, sabemos muito bem que ela nunca ira conseguir fazer em menos movs que a peça A. 
...
int think(vector<vector<int>> matriz, vector<int> Positions, int numberPos, int otherSteps) {
    int moves;
    int minMoves = INT_MAX;
    int standing;
    int numFails;

    int diffSteps = otherSteps - numberPos;
...
void letsDance(int numberPos, int start, vector<int> toCover, vector<vector<vector<int>>> brain) {
    ...
    steps[0] = think(brain[0], Positions, numberPos, INT_MAX);
    steps[1] = think(brain[1], Positions, numberPos, steps[0]);
    steps[2] = think(brain[2], Positions, numberPos, min(steps[0], steps[1]));
