...
// Info:
// VERSAO: Infelizmente fazendo o dito uso de floydWarshall (corrigido)
// 5 SUBMISSOES FEITAS APOS A SESSAO PROGRAMAÇAO COMPETITIVA
...
vector<vector<int>> matrizMovQ() {
    vector<vector<int>> d(8, vector<int>(8)); //(d)istancias
    ...
    for(int i = 0; i < 64; i++) {
        for(int j = 0; j < 64; j++) {
            if (i == j) {
                ...
                if(abs(rowI - rowJ) == abs(colI - colJ)) { // bispo
                    d[i][j] = 1;
                }
                else if((rowDiff == 2 && colDiff == 1) || (rowDiff == 1 && colDiff == 2)) { // cavalo
                    d[i][j] = 1;
                }
...
int think(vector<vector<int>>& matriz, vector<int>& Positions, int numberPos) {
    ...
    do {
        ...
        for (int i = 0; i < numberPos; i++) {
            moves += matriz[path[i]][path[i + 1]];
        }

        minMoves = min(minMoves, moves);
    } while (next_permutation(path.begin() + 1, path.end()) && minMoves > numberPos);
