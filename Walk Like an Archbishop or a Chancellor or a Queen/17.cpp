...
int convertToIndex(string Position){
   return (Position[0] - '1') + (Position[1] - '1') * 8;
}
...
int main(int argc, char const *argv[]) {
    ...
    for(int i = 0; i < games; i++){
        if(getline(cin, line)) {
            ...
            ss >> numberPos >> start2; //Colocar o S1 => numberPos; colocar o s2 => start

            start = convertToIndex(start2);
        }
        ...
        for(int i = 0; i < numberPos; i++){
            if (getline(cin, line)) {
                ...
                toCover[i] = convertToIndex(line);
                // ss >> toCover[i];

                // toCover[i][0] = convertLettersToNumber(toCover[i][0]);
                // Em falta: Converter tudo para index
            }
        }
