...
// Info:
// VERSAO: 
// 4 SUBMISSOES FEITAS APOS A SESSAO PROGRAMAÇAO COMPETITIVA
...
void floydWarshall(vector<vector<int>> &graph) {
    ...
    for (int k = 0; k < V; k++) {
        for (int i = 0; i < V; i++) {
            for (int j = 0; j < V; j++) {
                if ((graph[i][j] == -1 || graph[i][j] > (graph[i][k] + graph[k][j])) && (graph[k][j] != -1 && graph[i][k] != -1)) graph[i][j] = graph[i][k] + graph[k][j];
            }
        }
    }
}
...
int think(vector<vector<int>>& matriz, vector<int>& Positions, int numberPos) {
    ...
    for (int i = 0; i < numberPos; i++) {
        moves += matriz[Positions[i]][Positions[i + 1]];
    }
    return moves;
}
