...
public class Main {
    ...
    public static int printRequesitado(int N1, int N2, List<Integer> listaOrdenada1, List<Integer> listaOrdenada2){
        ...
        int maxDots = 0;
        for (int i = 0; i < listaOrdenada1.size(); i++) {
            int dot = 0;
            for (int j = 0; j < listaOrdenada1.size(); j++) {
                int dezenaI = listaOrdenada1.get(i) / 10;
                int dezenaJ = listaOrdenada1.get(j) / 10;

                //System.out.println("com o valores: " + listaOrdenada1.get(i) + " e " + listaOrdenada1.get(j) + " as dezenas são: " + dezenaI + " e " + dezenaJ);
                if (dezenaI == dezenaJ) {
                dot++;
                }
            }
        ...
        for(int i = 0; i < dezenasDiferentes; i++){
            ...
            if(listDezenas.get(i) == (listaOrdenada1.get(ordem1) / 10)){
            ...
            for(int j = ordem1 - 1; numeros > 0; numeros--, j--){
